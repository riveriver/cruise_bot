...
int main(int argc, char** argv){
  ...
  while (node.ok()){
    ...
    /* camera_init --> odom(camera_init_2d) */
    try{
    ...
    float x=transform_listener.getOrigin().x();
    float y=transform_listener.getOrigin().y();
    float z=0;
    float oriation_x=transform_listener.getRotation().getX();
    float oriation_y=transform_listener.getRotation().getY();
    float oriation_z=transform_listener.getRotation().getZ();
    float oriation_w=transform_listener.getRotation().getW();

    transform_broadcaster.setOrigin( tf::Vector3(x, y, 0.0) );
    transform_broadcaster.setRotation( tf::Quaternion(0, 0, oriation_z, oriation_w) );
    broadcaster.sendTransform(tf::StampedTransform(transform_broadcaster, ros::Time::now(), "map", "odom"));
    ...
    broadcaster.sendTransform(tf::StampedTransform(transform_broadcaster, ros::Time::now(), "map", "base_laser_link"));
